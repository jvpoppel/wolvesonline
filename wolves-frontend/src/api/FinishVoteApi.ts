...
export class FinishVoteApi {
  ...
  public static async send(gameToken: string, playerToken: string, uuid: string): Promise<{ status: string }> {
    if (this.sending) {
      console.warn("FinishVoteApi: Asked for new request but previous request is still sending!");
      return;
    }
    this.sending = true;

    return await new BaseApi().put<{ status: string }>("{0}:{1}/api/game/{2}/{3}/vote/finish"
