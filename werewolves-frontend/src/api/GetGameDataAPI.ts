...
export class GetGameDataAPI {
  ...
  public static async send(playerToken: string, gameToken: string, iteration: number): Promise<GameData> {
    if (this.sending) {
    ...
    return await new BaseApi().post<GameData>("{0}:{1}/api/game/{2}/{3}/data"
      ...
      .replace("{2}", gameToken).replace("{3}", playerToken), {"iteration": iteration+""})
      .then(response => {
        this.sending = false;
        return response;
      });
